...
protected:
    void test_capacity(Args &&...args) {
        ...
        static constexpr uint64_t TEST_SIZE = 100;
    ...
    void test_bbox_find(Args &&...args) {
        ...
        static constexpr int N_BBOX = 10;
    ...
    void test_nearest(Args &&...args) {
        ...
        st::KDTree<int> tree;
        ASSERT_EQ(tree.nearest(0, 0), tree.end());
        ...
        for (int i = 0; i < TEST_SIZE; ++i) {
            ...
            const auto [nearest_x2, nearest_y2, nearest_val2] = *tree.nearest(x_, y_);

            const CoordT dist1 = st::euclidean_distance_squared(nearest_x1, x_, nearest_x1, y_);
            const CoordT dist2 = st::euclidean_distance_squared(nearest_x2, x_, nearest_x2, y_);

            if constexpr (std::is_floating_point_v<CoordT>) {
                ASSERT_DOUBLE_EQ(dist1, dist2);
            } else {

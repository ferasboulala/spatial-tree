...
TEST(TestSpatialTree, Destructors) {
    int counter = 0;
    st::internal::spatial_tree<int, MyObject, 2, 1> tree;
...
TEST(TestSpatialTree, IncreasingVolume) {
    st::internal::spatial_tree<int, int, 3> tree;
...
TEST(TestSpatialTree, UniqueInsertions) {
    const auto typed_test = [&]<typename CoordT>() {
        ...
        for (uint64_t test_size : test_sizes) {
            ...
            inner_test.template operator()<st::internal::spatial_tree<CoordT, int,
            2>>(unbounded);
            ...
            inner_test.template operator()<st::internal::spatial_tree<CoordT, int, 2,
            7>>(odd_size);
        }
    };
    typed_test.operator()<float>();
...
TEST(TestSpatialTree, IteratorCoverage) {
    ...
    const auto                  typed_test = [&]<typename CoordT>() {
        for (int test_size : test_sizes) {
            ...
            const st::bounding_box<CoordT, 2> bounds = {-boundary, -boundary, boundary,
            boundary};
                ...
                for (uint64_t i = 0; i < test_size; ++i) {
                    ...
                    const auto [pt, nearest_val1] =
                        *std::min_element(tree.begin(), tree.end(), [&](auto lhs, auto rhs) {
                            ...
                            <
                                   st::internal::euclidean_distance_squared(p2[0], x_, p2[1],
                                   y_);
                        });
